...
dataset = pd.read_csv('data_v2.csv', sep=',')
# header = list(dataset.columns)
# print(header)
numeric_features = ['Días de cultivo celular', '% Confluencia', 'Cantidad de Pasajes bool', 'Cantidad de Pasajes numeric', 'Tiempo con bajo suero (DMEM 0,5% SFB) en horas','% Maduración','% Clivaje','Día cambio de medio','ø Eje menor', 'ø Eje mayor','Días post-ovulación']
...
clf1 = setup(data=dataset,
             ...
             ignore_features=['Fecha Transferencia', 'Fecha Producción', 'Fecha Descongelamiento','Línea celular', 'Yegua receptora N°'],
...
model_name = ['rf', 'qda', 'ada', 'gbc', 'lda', 'et', 'xgboost', 'lightgbm', 'catboost', 'dummy']
# print(model_name)
for i in range(len(model_name)):
    ...
    shutil.copyfile(confusion_matrix, "/media/rodrigo/Data1/Kheiron/kheiron_tabular/compare_models_v2/" + model_name[i]+"_confusion_matrix.png")
    shutil.copyfile(auc, "/media/rodrigo/Data1/Kheiron/kheiron_tabular/compare_models_v2/" + model_name[i]+"_auc.png")
    metrics = pull()
    metrics.to_csv("/media/rodrigo/Data1/Kheiron/kheiron_tabular/compare_models_v2/"+model_name[i]+"_metrics.csv")
